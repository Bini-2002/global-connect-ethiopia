...
```powershell
py -3.11 -m venv venv.venvScriptsActivatepython -m pip install --upgrade pip
```
...
```dotenv
MONGODB_URL="mongodb+srv://<username>:<password>@globalconnectcluster.m4yhahs.mongodb.net/?appName=GlobalConnectCluster"PROJECT_NAME="Global Connect"VERSION="1.0.0"DATABASE_NAME="global_connect_db"JWT_SECRET="your_jwt_secret_key_here"ALGORITHM="HS256"ACCESS_TOKEN_EXPIRE_MINUTES=30
```
...
```bash
git add .git commit -m "Add feature XYZ"git push origin feature/<branch-name>
```