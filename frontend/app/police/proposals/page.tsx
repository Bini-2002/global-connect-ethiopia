...
export default function PoliceProposalsPage() {
  ...
  useEffect(() => {
    api
      .get<PoliceNotificationRecord[]>("/police/proposals/")
  ...
  const filtered = notifications.filter(
    (item) =>
      ...
      (item.organizer_contact.organizer_name || "")
        .toLowerCase()
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 animate-fade-in">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">
              Persisted, read-only security notifications created automatically
          ...
          {loading ? (
            ...
            <div className="py-16 text-center">
              <p className="text-slate-500 mt-3 text-sm">
                No approved event notifications found.
              </p>
            </div>
          ) : (
            <div className="divide-y divide-slate-100">
              {filtered.map((item) => (
                <div
                  key={item.id}
                  className="grid grid-cols-12 gap-4 items-center px-6 py-4 hover:bg-slate-50 transition"
                >
                  <div className="col-span-4">
                    ...
                    <p className="text-xs text-slate-500 mt-1">
                      {item.police_office.office_name ||
                  ...
                  <div className="col-span-2 text-sm text-slate-600">
                    {item.expected_attendees?.toLocaleString() || "—"}
                  </div>
                  ...
                  <div className="col-span-1 flex justify-end">
                    <Link
                      href={`/police/proposals/${item.proposal_id}`}
                      className="text-xs border border-slate-200 px-3 py-1.5 rounded-lg hover:bg-[#062E22] hover:text-white hover:border-[#062E22] transition font-medium"
                    >
                      Open
                    </Link>
                  </div>
