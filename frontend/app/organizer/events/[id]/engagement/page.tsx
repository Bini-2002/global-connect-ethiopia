...
import { BellRing, PlayCircle, BadgeInfo, Download } from 'lucide-react';
...
export default function EventEngagementPage() {
  ...
  const [runningAnnouncement, setRunningAnnouncement] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const [announcementsResponse, badgesResponse] = await Promise.all([
          eventsService.getAnnouncements(eventId),
          eventsService.getBadges(eventId).catch(() => []),
        ]);
        ...
        if (!selectedAnnouncementId && announcementsResponse[0]) {
          setSelectedAnnouncementId(announcementsResponse[0].id);
        }
      } catch (err) {
        if (active) setError(err instanceof Error ? err.message : 'Failed to load engagement data');
      } finally {
        if (active) {
  ...
  const handleAnnouncement = async () => {
    ...
    try {
      ...
      if (created.status === 'sent') {
        const deliveryResponse = await eventsService.getAnnouncementDeliveries(event.id, created.id);
  ...
  const handleRunNow = async (announcementId: string) => {
    ...
    try {
      ...
      setError(null);
      const response = await eventsService.runScheduledAnnouncementNow(event.id, announcementId);
      setAnnouncements((current) =>
        current.map((item) => (item.id === announcementId ? response.announcement : item))
      );
      setSelectedAnnouncementId(announcementId);
      setDeliveries(response.deliveries);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to run scheduled announcement');
    } finally {
  ...
  const handleGenerateBadges = async () => {
    ...
    try {
      ...
      setError(null);
      const newBadges = await eventsService.generateBadges(event.id, {
        include_unchecked_in: true,
      });
      
      const badgeMap = new Map(badges.map(b => [b.id, b]));
      newBadges.forEach(b => badgeMap.set(b.id, b));
      
      setBadges(Array.from(badgeMap.values()).sort((a, b) => new Date(b.generated_at).getTime() - new Date(a.generated_at).getTime()));
    } catch (err) {
  ...
  return (
      ...
      aside={
        <div className="space-y-6">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            <h2 className="text-lg font-bold text-[#062E22]">Engagement Snapshot</h2>
            <div className="grid grid-cols-2 gap-3 mt-4">
              ...
              <div className="rounded-xl bg-slate-50 p-4">
                ...
                <p className="text-xl font-bold text-[#062E22] mt-2">{deliveries.length}</p>
              </div>
              <div className="col-span-2 rounded-xl bg-slate-50 p-4">
                <p className="text-xs uppercase tracking-wide text-slate-400">Badges Generated</p>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            ...
            <p className="text-sm text-slate-600 mt-4">
              This phase only delivers in-app announcements to confirmed bookings. Scheduled messages stay dormant until you run them manually here.
            </p>
          </div>
        </div>
      }
    >
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 lg:col-span-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-[#062E22]">Attendee Badges</h2>
                <p className="text-sm text-slate-500">Generate virtual badges for all confirmed bookings to be used during the event.</p>
              </div>
            </div>
            <button
            ...
            {loadingBadges ? (
              ...
              <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
                {badges.slice(0, 6).map((badge) => (
                  <div key={badge.id} className="rounded-2xl border border-slate-200 p-4 bg-slate-50 flex items-center justify-between">
                    <div>
                      <p className="font-semibold text-[#062E22] truncate w-32">{badge.attendee_name || badge.attendee_email}</p>
                      <p className="text-xs text-slate-500 mt-1">{badge.role_label}</p>
                    </div>
                    <div className="text-xs font-mono bg-white px-2 py-1 border border-slate-200 rounded text-slate-500">
                      {badge.badge_code}
                    </div>
                  </div>
                ))}
                {badges.length > 6 && (
                  <div className="rounded-2xl border border-dashed border-slate-200 p-4 flex items-center justify-center text-sm text-slate-500">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-[#062E22]">Create Announcement</h2>
              <p className="text-sm text-slate-500">Immediate sends execute now. Future sends are saved as scheduled records.</p>
            </div>
          </div>
          ...
          <div className="space-y-4 mt-6">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-2">
                Leave blank to deliver immediately. Future dates create scheduled records.
              </p>
            </div>
            <button
        ...
        {/* Announcement Records Section */}
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
          <h2 className="text-lg font-bold text-[#062E22]">Announcement Records</h2>
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-2">
                        {announcement.send_at ? `Scheduled for ${formatDateTime(announcement.send_at)}` : `Created ${formatDateTime(announcement.created_at)}`}
                      </p>
                    </div>
                    {announcement.status === 'scheduled' ? (
                      ...
                      >
                        ...
                        {runningAnnouncement === announcement.id ? 'Running...' : 'Run Now'}
                      </button>
                    ) : null}
                  </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 mt-6">
        <h2 className="text-lg font-bold text-[#062E22]">Resolved In-App Deliveries</h2>
            ...
            {deliveries.map((delivery) => (
              <div key={delivery.id} className="rounded-2xl border border-slate-200 p-4">
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
                  <div>
                    <p className="font-semibold text-[#062E22]">{delivery.recipient_name || delivery.recipient_email || delivery.recipient_user_id}</p>
                    <p className="text-sm text-slate-500 mt-1">{delivery.recipient_email || 'No email on record'}</p>
                    <p className="text-sm text-slate-600 mt-3">{delivery.body}</p>
                  </div>
                  <div className="rounded-xl bg-slate-50 px-4 py-3 text-sm text-slate-600">
                    <p>{sentenceCase(delivery.status)}</p>
                    <p className="mt-1">{formatDateTime(delivery.delivered_at || delivery.created_at)}</p>
                  </div>
                </div>
              </div>
            ))}
