...
export default function ProposalDetailPage() {
  ...
  useEffect(() => {
    api
      .get<ProposalRecord>(`/proposals/${id}`)
      .then(setProposal)
  ...
  const handleEdit = () => {
    ...
    localStorage.setItem(
      "pendingProposal",
      JSON.stringify(proposalToSessionData(proposal)),
    );
    router.push("/organizer/proposals/create");
  };

  const canEdit =
    proposal && ["draft", "changes_requested"].includes(proposal.status);
  const canSubmit =
    proposal && ["draft", "changes_requested"].includes(proposal.status);
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
              <div>
                <div className="flex items-center gap-3 mb-1">
                  {(() => {
                    const cfg = PROPOSAL_STATUS_META[proposal.status] || {
                    ...
                    return (
                      <span
                        className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${cfg.cls}`}
                      >
                        <span
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                {isApproved && (
                  <Link
                    href={`/organizer/proposals/${id}/permit`}
                ...
                {isApproved && (
                  <Link
                    href={
                      proposal.event_id
                        ? `/organizer/events/${proposal.event_id}`
                        : `/organizer/create-event/${id}`
                    }
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {(proposal.target_audience || []).map((audience) => (
                            <span
                              key={audience}
                              className="px-2.5 py-1 rounded-full bg-green-50 border border-green-200 text-xs font-medium text-green-700"
                            >
                              {audience}
                            </span>
                          ))}
                          {!proposal.target_audience?.length && (
                  ...
                  <div className="flex items-center gap-2 text-xs font-semibold text-slate-400 uppercase tracking-wide mb-4">
                    ...
                    >
                      <path
                        ...
                        strokeLinejoin="round"
                        strokeWidth="2"
                        d="M9 20l-5.447-2.724A2 2 0 013 15.382V5.618a2 2 0 011.553-1.894l4-1A2 2 0 019 2.764m0 17.236l6-2m-6 2V2.764m6 15.236l5.447-2.724A2 2 0 0021 13.382V3.618a2 2 0 00-1.553-1.894l-4-1A2 2 0 0015 0.764m0 17.236V0.764m0 0L9 2.764"
                      />
                    </svg>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="rounded-xl bg-slate-50 border border-slate-200 p-4">
                      ...
                      <p className="font-semibold text-[#062E22]">
                        {getOfficeLabel(proposal.office_assignments?.ministry)}
                      </p>
                      <p className="text-xs text-slate-500 mt-2">
                        {proposal.office_assignments?.ministry?.email ||
                          "No ministry office assigned"}
                      </p>
                    </div>
                    <div className="rounded-xl bg-slate-50 border border-slate-200 p-4">
                      <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-1">
                      ...
                      <p className="text-xs text-slate-500 mt-2">
                        {proposal.office_assignments?.municipal?.email ||
                          "No municipal office assigned"}
                      </p>
                    </div>
                    <div className="rounded-xl bg-slate-50 border border-slate-200 p-4">
                      <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-1">
                      ...
                      <p className="text-xs text-slate-500 mt-2">
                        {proposal.office_assignments?.police?.email ||
                          "No police notification office assigned"}
                      </p>
                    </div>
                  </div>
                </div>
                    ...
                    <div className="flex items-center gap-2 text-xs font-semibold text-slate-400 uppercase tracking-wide mb-4">
                      ...
                      >
                        <path
                          ...
                          strokeLinejoin="round"
                          strokeWidth="2"
                          d="M7 8h10M7 12h6m-6 4h10M5 3h14a2 2 0 012 2v14l-4-3-4 3-4-3-4 3V5a2 2 0 012-2z"
                        />
                      </svg>
                      Organizer Updates
                    </div>
                        ...
                        .map((update, index) => (
                          ...
                          >
                            <div className="flex items-center justify-between gap-3">
                              <p className="font-semibold text-[#062E22]">
                                {update.office_name ||
                                  formatProposalStage(update.stage)}
                              </p>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                  ...
                  <p className="text-sm font-semibold text-[#062E22]">
                    {formatProposalStage(proposal.status)}
                  </p>
                ...
                {proposal.approval_certificate_number && (
                  <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                    ...
                    <p className="text-lg font-bold text-[#062E22]">
                      {proposal.approval_certificate_number}
                    </p>
                    <p className="text-xs text-slate-500 mt-2">
                      Issued after municipal approval.
                    </p>
                    <Link
                      href={`/organizer/proposals/${id}/permit`}
                      className="inline-flex mt-4 text-sm font-semibold text-green-700 hover:underline"
                    >
                    ...
                    {TIMELINE_STEPS.map((step, i) => {
                      ...
                      return (
                        <div key={step.key} className="flex items-center gap-3">
                          <div
                            className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${isDone ? "bg-[#062E22]" : "bg-slate-200"}`}
                          >
                            {isDone ? (
                              <svg
                          ...
                          <div>
                            <p
                              className={`text-xs font-medium ${isDone ? "text-[#062E22]" : "text-slate-400"}`}
                            >
                              {step.label}
                            </p>
                            {isCurrent && (
