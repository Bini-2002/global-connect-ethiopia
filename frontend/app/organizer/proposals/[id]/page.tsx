          ...
          <div className="max-w-5xl mx-auto animate-fade-in">
            ...
            <div className="grid grid-cols-3 gap-6">
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5">
                        {proposal.visibility === 'private' ? (
                          <>
                            ...
                            <span className="text-sm font-medium text-amber-700">Private (Invitation Only)</span>
                          </>
                        ) : (
                          <>
                            <Globe className="w-3.5 h-3.5 text-blue-600" />
                            <span className="text-sm font-medium text-blue-700">Public Event</span>
                          </>
                        )}
                      </div>
                    </div>
              ...
              <div className="space-y-6">
                ...
                {(proposal.approval_certificate_number || proposal.verification_letter_reference) && (
                  <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                    <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-2">Approval Artifacts</p>
                    <p className="text-lg font-bold text-[#062E22]">{proposal.approval_certificate_number || 'Permit pending'}</p>
                    <p className="text-sm text-slate-600 mt-2">
                      Verification letter: {proposal.verification_letter_reference || 'Pending'}
                    </p>
                    <p className="text-xs text-slate-500 mt-2">Both records are issued after municipal approval.</p>
                    <Link href={`/organizer/proposals/${id}/permit`} className="inline-flex mt-4 text-sm font-semibold text-green-700 hover:underline">
                      Open approval documents
                    </Link>
                  </div>
                )}
