...
export default function TeamDashboardPage() {
  ...
  const handleWithdrawConfirm = async () => {
    ...
    try {
      ...
      await refreshTx();
      alert(
        "Withdrawal successful! Funds have been sent to your registered Chapa account.",
      );
    } catch (err) {
      const msg =
  ...
  const stats = {
    pending: tasks.filter(
      (t) => t.status === "open" || t.status === "pending_approval",
    ).length,
    ...
    totalEarned: transactions
      .filter(
        (tx) =>
          tx.transaction_type === "TASK_PAYOUT" || tx.type === "TASK_PAYOUT",
      )
        ...
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:flex flex-col items-end mr-4">
              ...
              <p className="text-lg font-bold text-[#8CB988]">
                {formatCurrency(wallet?.balance || wallet?.available_balance)}
              </p>
            </div>
            <button
              onClick={() => {
            ...
            <div className="bg-[#062E22] rounded-[2.5rem] p-8 shadow-2xl shadow-emerald-900/20 text-white relative overflow-hidden group">
              ...
              <div className="relative z-10">
                ...
                <button
                  ...
                  disabled={
                    isWithdrawing ||
                    (wallet?.balance || wallet?.available_balance || 0) <= 0
                  }
                >
              ...
              <div className="divide-y divide-slate-50">
                {loadingTx ? (
                  <div className="p-8 text-center animate-pulse">
                    ...
                    (t) =>
                      t.transaction_type === "TASK_PAYOUT" ||
                      t.type === "TASK_PAYOUT",
                  ).length === 0 ? (
            ...
            <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden min-h-[600px]">
              <div className="p-8 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-xl font-black text-[#062E22]">
                      Active Assignments
                    </h2>
                ...
                {loadingTasks ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-32 text-center">
                    ...
                    <p className="text-slate-400 text-sm max-w-xs mx-auto font-medium">
                      When organizers assign you tasks, they'll appear here
                      automatically.
                    </p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 gap-4">
                    {tasks.map((task) => (
                      <div
                        key={task.id}
                        className="group p-6 bg-white hover:bg-slate-50 border border-slate-100 rounded-[2rem] transition-all duration-300 hover:shadow-xl hover:shadow-slate-200/50"
                      >
                        <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
                          <div className="flex-1">
                            <div className="flex flex-wrap items-center gap-3 mb-3">
                              <h3 className="text-lg font-black text-[#062E22] group-hover:text-black transition-colors">
                                {task.title}
                              </h3>
                              <span
