...
export default function AdminVendorsPage() {
  ...
  const filtered = apps.filter(a =>
    ...
    (a.business_category || '').toLowerCase().includes(query.toLowerCase())
  );
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <Sidebar role="admin" />
      <DashboardHeader searchPlaceholder="Search vendors..." onSearch={setQuery} />
      <main className="ml-60 pt-16 p-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 animate-fade-in">
          <div>
            <h1 className="text-2xl font-bold text-[#062E22]">Vendor Review Queue</h1>
            <p className="text-slate-500 text-sm mt-1">Review pending vendor registration and verification applications.</p>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 animate-fade-in delay-100">
          ...
          {loading ? (
            ...
            <div className="py-16 text-center"><span className="text-4xl">✅</span><p className="text-slate-500 mt-3 text-sm">No pending vendor applications.</p></div>
          ) : (
                ...
                return (
                  <div key={a.id} className="grid grid-cols-12 gap-4 items-center px-6 py-4 hover:bg-slate-50 transition">
                    <div className="col-span-3">
                      <p className="font-semibold text-[#062E22] text-sm">{a.business_name || 'N/A'}</p>
                      <p className="text-xs font-mono text-slate-400">{a.user_id.substring(0, 8)}</p>
                    </div>
                    <div className="col-span-2 text-sm text-slate-600">{a.business_category || '—'}</div>
                    ...
                    <div className="col-span-1 text-right"><Link href={`/admin/vendors/${a.id}`} className="text-xs border border-slate-200 px-3 py-1.5 rounded-lg hover:bg-[#062E22] hover:text-white hover:border-[#062E22] transition font-medium">Review</Link></div>
                  </div>
                );
              })}
            </div>
