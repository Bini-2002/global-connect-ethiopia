...
export default function MunicipalProposalsPage() {
  ...
  const filtered = proposals.filter(p => {
    if (tab === 'pending' && !['ministry_approved', 'municipal_review'].includes(p.status)) return false;
    if (tab === 'approved' && p.status !== 'approved') return false;
    if (tab === 'rejected' && p.status !== 'rejected') return false;
    return p.title.toLowerCase().includes(query.toLowerCase());
  });
  const pendingCount = proposals.filter(p => ['ministry_approved', 'municipal_review'].includes(p.status)).length;
      ...
      <main className="ml-60 pt-16 p-8">
        ...
        <div className="flex gap-2 mb-6 animate-fade-in delay-100">
          {[
            ...
            { key: 'pending', label: 'Submitted (Waiting Municipal)', count: pendingCount },
              ...
              {filtered.map(p => (
                <div key={p.id} className="grid grid-cols-12 gap-4 items-center px-6 py-4 hover:bg-slate-50 transition">
                  ...
                  <div className="col-span-1 text-right">
                    <Link href={`/municipal/proposals/${p.id}`} className="text-xs border border-slate-200 px-3 py-1.5 rounded-lg hover:bg-[#062E22] hover:text-white hover:border-[#062E22] transition font-medium">Open</Link>
                  </div>
                </div>
              ))}
            </div>
